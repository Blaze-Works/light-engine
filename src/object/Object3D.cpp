...
void Object3D::lookAt(float x, float y, float z) {
	...
	quaternion = glm::normalize(glm::quat_cast(glm::inverse(m)));
...
void Object3D::updateMatrix() {
	if (this->sameAsSnapshot()) return;
	this->matrix = glm::translate(glm::mat4(1.0f), this->position) * glm::mat4_cast(this->quaternion) * glm::scale(glm::mat4(1.0f), this->scale);
...
bool Object3D::sameAsSnapshot() {
	return this->position == this->positionSnap && this->quaternion == this->quaternionSnap && this->scale == this->scaleSnap;
}

void Object3D::updateSnapshot() {
	this->positionSnap = this->position;
	this->quaternionSnap = this->quaternion;
	this->scaleSnap = this->scale;
}
