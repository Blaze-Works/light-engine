...
void update(bl::Window& window, float deltaTime) {
    ...
    if (bl::Keyboard::isPressed(bl::Keyboard::Down) && (pad1->getY() - 150.f / 2 > 5.f)) {
        pad1->move(0.f, -paddleSpeed * deltaTime);
    }

    if (bl::Keyboard::isPressed(bl::Keyboard::Up) && (pad1->getY() + 150.f / 2 < window.getHeight() - 5.f)) {
        pad1->move(0.f, paddleSpeed * deltaTime);
    }
...
void render(bl::DrawContext& context, int mouseX, int mouseY, float delta) {
    ...
    if (!isPlaying && bl::Keyboard::isPressed(bl::Keyboard::Space)) {
        init(*window);
        isPlaying = true;
    }
