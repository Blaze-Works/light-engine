...
public:
	...
	Box(float x, float y, float w, float h, bool isCircle = false) {
...
static float paddleSpeed = 80;
static float leftPaddleSpeed;
...
void init(bl::Window& window) {
	...
	ball = new Box((float) w / 2 - 15, (float) h / 2 - 15, 10, 10, true);
	pad1 = new Box(x, y, 30, 150);
	pad2 = new Box(w - x - 30, y, 30, 150);
	ballSpeed = 50;

	AITimer = new bl::Clock();

	window.onKeyDown([&window](int key) {
		if (key == bl::Keyboard::Esc) bl::LightEngine::getInstance().scheduleStop();
		if (!isPlaying && key == bl::Keyboard::Space) { init(window); isPlaying = true; }
		if (key == bl::Keyboard::LeftShift && (pad1->getY() - 150.f / 2 > 5.f)) leftPaddleSpeed = -paddleSpeed;
		if (key == bl::Keyboard::LeftAlt && (pad1->getY() + 150.f / 2 < window.getHeight() - 5.f)) leftPaddleSpeed = paddleSpeed;
	});
...
void update(bl::Window& window, float deltaTime) {
	...
	if (((leftPaddleSpeed < 0.f) && (pad2->getY() - 150.f / 2 > 5.f)) || ((leftPaddleSpeed > 0.f) && (pad2->getY() + 150.f / 2 < window.getHeight() - 5.f))) {
		pad1->move(0.f, leftPaddleSpeed * deltaTime);
	}
	...
	if (AITimer->getElapsedTime().asMicroseconds() > AITime.asMicroseconds()) {
		AITimer->restart();
		if (ball->getY() + ballRadius > pad2->getY() + 150.f / 2) rightPaddleSpeed = paddleSpeed * ballSpeed / 20;
		else if (ball->getY() - ballRadius < pad2->getY() - 150.f / 2) rightPaddleSpeed = -paddleSpeed * ballSpeed / 20;
...
int main() {
	...
	engine.addUpdateCallback([&window](float delta) {
		if (isPlaying) update(window, delta);
	});
